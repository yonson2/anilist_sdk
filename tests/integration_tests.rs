...
async fn test_comprehensive_integration() {
    ...
    let popular_anime = client.anime().get_popular(1, 3).await.expect("Failed to get popular anime");
    assert!(!popular_anime.is_empty());
    println!("✓ Popular anime: Found {} entries", popular_anime.len());
    
    let trending_anime = client.anime().get_trending(1, 3).await.expect("Failed to get trending anime");
    ...
    if let Some(first_anime) = popular_anime.first() {
        let anime_by_id = client.anime().get_by_id(first_anime.id).await.expect("Failed to get anime by ID");
        assert_eq!(anime_by_id.id, first_anime.id);
        println!("✓ Anime by ID: Successfully retrieved anime {}", first_anime.id);
    }
    
    // Test manga endpoints
    println!("Testing manga endpoints...");
    
    let popular_manga = client.manga().get_popular(1, 3).await.expect("Failed to get popular manga");
    assert!(!popular_manga.is_empty());
    println!("✓ Popular manga: Found {} entries", popular_manga.len());
    
    let trending_manga = client.manga().get_trending(1, 3).await.expect("Failed to get trending manga");
    ...
    let popular_characters = client.character().get_popular(1, 3).await.expect("Failed to get popular characters");
    assert!(!popular_characters.is_empty());
    println!("✓ Popular characters: Found {} entries", popular_characters.len());
    
    // Test staff endpoints
    println!("Testing staff endpoints...");
    
    let popular_staff = client.staff().get_popular(1, 3).await.expect("Failed to get popular staff");
    ...
    let anime_search = client.anime().search("Attack on Titan", 1, 2).await.expect("Failed to search anime");
    assert!(!anime_search.is_empty());
    println!("✓ Anime search: Found {} entries for 'Attack on Titan'", anime_search.len());
    
    let character_search = client.character().search("Eren", 1, 2).await.expect("Failed to search characters");
    assert!(!character_search.is_empty());
    println!("✓ Character search: Found {} entries for 'Eren'", character_search.len());
...
async fn test_pagination() {
    ...
    let page1 = client.anime().get_popular(1, 5).await.expect("Failed to get page 1");
    let page2 = client.anime().get_popular(2, 5).await.expect("Failed to get page 2");
    ...
    assert!(has_different_content, "Pages should contain different content");
