...
async fn test_current_user_without_token() {
    ...
    assert!(result.is_err(), "Current user endpoint should fail without authentication");
}
        ...
        if !token.is_empty() && token != "fake_token" {
            ...
            if let Ok(_user) = user_result {
                ...
                match anime_list_result {
                    Ok(anime_list) => {
                        println!("✓ Successfully retrieved anime list with {} entries", anime_list.len());
                    ...
                    Err(e) => {
                        ...
                        println!("Note: Could not retrieve anime list (might be private): {:?}", e);
                    }
                }
            } else {
